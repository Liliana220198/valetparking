import math #proporciona acceso a las funciones matematicas predefinidas
...
def linear_Congruential_Method(Xo, a, c, m, noOfRandomNums):
    ...
    for i in range(1,noOfRandomNums):
        randomNums[i] = round(((randomNums[i-1] * a) + c) % m , 4) #formula de congruencia lineal para generar numeros pseudoaleatorios y cada valor lo guarda en randomNums[i]
...
def cliente(name):
    ...
    if(name > 0): #if para saber si es diferente del primer cliente
        ...
        if(tiemposClientes[name -1]['salida'] > tiemposClientes[name]['llegada'] ):
            ...
            '''
            espera = tiemposClientes[name -1]['salida'] - tiemposClientes[name]['llegada'] #se le resta la hora de llegada a la hora de salida del cliente anterior para calcular el tiempo de espera
            '''
...
if __name__ == "__main__":
    ...
    for i in range(total_espacios): #se crea una se secuencia de numeros de 0 hasta total_espacios
        ...
        llegada += -t_llegadas * math.log(R) # formula para calcular el tiempo de llegada y se acomula en la variable llegada
